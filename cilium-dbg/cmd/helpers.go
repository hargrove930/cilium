...
func updatePolicyKey(pa *PolicyUpdateArgs, add bool) {
	...
	for _, proto := range pa.protocols {
		...
		if add {
			...
			if pa.isDeny {
				err = policyMap.Deny(pa.label, pa.port, 0xffff, u8p, pa.trafficDirection)
			} else {
				err = policyMap.Allow(pa.label, pa.port, 0xffff, u8p, pa.trafficDirection, authType, proxyPort)
			}
			if err != nil {
				Fatalf("Cannot add policy key '%s': %s\n", entry, err)
			}
		} else {
			if err := policyMap.Delete(pa.label, pa.port, 0xffff, u8p, pa.trafficDirection); err != nil {
