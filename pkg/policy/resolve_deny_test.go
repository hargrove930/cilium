...
func (ds *PolicyTestSuite) TestMapStateWithIngressDeny(c *C) {
	...
	expectedEndpointPolicy := EndpointPolicy{
		...
		policyMapState: newMapState(map[Key]MapStateEntry{
			...
			// will still allow egress to world.
			{PortMask: 0xffff, TrafficDirection: trafficdirection.Egress.Uint8()}:                              allowEgressMapStateEntry,
			{PortMask: 0xffff, Identity: uint32(identity.ReservedIdentityWorld), DestPort: 80, Nexthdr: 6}:     rule1MapStateEntry.WithOwners(cachedSelectorWorld),
			{PortMask: 0xffff, Identity: uint32(identity.ReservedIdentityWorldIPv4), DestPort: 80, Nexthdr: 6}: rule1MapStateEntry.WithOwners(cachedSelectorWorld),
			{PortMask: 0xffff, Identity: uint32(identity.ReservedIdentityWorldIPv6), DestPort: 80, Nexthdr: 6}: rule1MapStateEntry.WithOwners(cachedSelectorWorld),
			{PortMask: 0xffff, Identity: 192, DestPort: 80, Nexthdr: 6}:                                        rule1MapStateEntry,
			{PortMask: 0xffff, Identity: 194, DestPort: 80, Nexthdr: 6}:                                        rule1MapStateEntry,
		}),
	}
